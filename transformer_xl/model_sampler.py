...
def sample_sequence(model, length, start_token=None, batch_size=None, context=None, temperature=1, top_k=0, device='cuda', sample=True):
    ...
    with torch.no_grad():
        for i in trange(length):
            predictions, past = model(output, mems=past)
            predicted_index = torch.topk(predictions[0, -1, :],1)[1][0].item()
...
def print_samples(model, enc, args, context_tokens=[], unconditional=True, **kwargs):
    ...
    for _ in range(kwargs['nsamples'] // kwargs['batch_size']):
        out = sample_sequence(
            ...
            start_token=enc.sym2idx['<eos>'] if unconditional else None,
        ...
        for i in range(kwargs['batch_size']):
            ...
            text = enc.decode(out[i][:len(context_tokens)])
            print(text)
            print("--")
            print(enc.decode(out[i][len(context_tokens):]))
    return text