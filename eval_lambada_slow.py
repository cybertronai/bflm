...
def main():
    ...
    if args.compare:
        ds_raw = open(f'{args.compare}').read()
        if args.preprocess:
            ds_raw = preprocess(ds_raw)
        regular_lines = ds_raw.strip().split('\n')
    ...
    if args.compare:
        comparisons_file = open('/ncluster/data/lambada_comparison.txt', 'w')
    ...
    for i, line in enumerate(lines):
        ...
        if args.compare:
            line_old = regular_lines[i]
            line_old = detokenizer.detokenize(line_old.split())
            line_old = line_old.replace(" n't", "n't")
            line_old = '\n'+line_old
            predicted_word_old, last_word_old = predict_wrapper(line_old)
            is_error_old = predicted_word_old.lower() != last_word_old.lower()
            if is_error_old and not is_error:
                comparisons_file.write(line_old+'>>>>\n')
                comparisons_file.write(predicted_word_old+'|||\n')
                comparisons_file.write(line+'>>>>\n')
                comparisons_file.write(predicted_word+'|||\n')
                comparisons_file.write('-'*80)
                comparisons_file.write('\n\n')
